  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 w-full transition-shadow hover:shadow-md">
      ...
      <div className="mt-3 h-3 bg-gray-100 rounded-full overflow-hidden">
        <div className="h-3 bg-blue-600 rounded-full" style={{ width: `${Math.max(0, Math.min(100, progress))}%` }} />
      </div>
      <button onClick={() => onOpen && onOpen(day)} className="mt-3 text-sm text-gray-600">View</button>
    </div>
  );
}
