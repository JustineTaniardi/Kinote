  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 max-w-sm w-full transition-shadow hover:shadow-md">
      ...
      <div className="mt-6 flex items-center justify-between">
        <button onClick={() => onOpen(entry)} className="px-4 py-2 border rounded-lg text-sm">Detail</button>
        <button onClick={() => onStart && onStart(entry)} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm">Start</button>
      </div>
    </div>
  );
}
